...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        foreach ($tables as $table) {
            ...
            $has_foreign_keys = isset($foreign_keys[$table]);
            foreach ($results as &$result) {
                ...
                if ($has_foreign_keys && isset($foreign_keys[$table][$result['Field']])) {
                    $result = array_merge($result, $foreign_keys[$table][$result['Field']]);
                }
            }
...
function get_foreign_keys($DB)
{
    $info_sql = 'SELECT `TABLE_NAME`, `COLUMN_NAME`, `REFERENCED_TABLE_NAME`, `REFERENCED_COLUMN_NAME` FROM INFORMATION_SCHEMA.`KEY_COLUMN_USAGE` WHERE `REFERENCED_TABLE_NAME` is not null ';
    $query = $DB->prepare($info_sql);
    $query->execute();
    ...
    if (!empty($foreignKeys)) {
        foreach ($foreignKeys as $data) {
            $result[$data ["TABLE_NAME"]][$data ["COLUMN_NAME"]] = [
                "Foreign key" =>  "[".$data ["REFERENCED_TABLE_NAME"]. "](#".$data ["REFERENCED_TABLE_NAME"].")." . $data ["REFERENCED_COLUMN_NAME"]
            ];
        }
    }
    return $result;
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <?php if ($config['comment_row']) : ?>
                        <br><br>
                        <p class="alert alert-warning"><b>Note:</b> Comments on separate rows will only appear in the
                            HTML version. The Markdown version will have the comment column included as normal.</p>
                    <?php endif; ?>
                    <h3>Markdown Results</h3>
                    <div id="tabs" role="tabpanel">
                        <ul class="nav nav-tabs" role="tablist">
                            <li role="presentation" class="active"><a href="#preview" role="tab" data-toggle="tab">Preview</a>
                            </li>
                            ...
                            <li role="presentation"><a href="#html_preview" role="tab" data-toggle="tab">HTML
                                    Preview</a></li>
                        </ul>
